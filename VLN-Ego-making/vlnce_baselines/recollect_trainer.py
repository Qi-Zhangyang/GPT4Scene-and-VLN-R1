...
class RecollectTrainer(BaseVLNCETrainer):
    ...
    def train(self) -> None:
        split = self.config.TASK_CONFIG.DATASET.SPLIT   # 'train'
        self.config.defrost()
        self.config.TASK_CONFIG.TASK.NDTW.SPLIT = split # NDTW: Normalized Dynamic Time Warping
        self.config.TASK_CONFIG.TASK.SDTW.SPLIT = split # SDTW: Success weighted by Dynamic Time Warping
